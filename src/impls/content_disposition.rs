...
use http::header::{self, HeaderName, HeaderValue, CONTENT_DISPOSITION};
...
impl Header for ContentDisposition {
    ...
    ) -> Result<Option<ContentDisposition>, Error> {
        if let Some(value) = values.next() {
            let s: String = value.to_str().map_err(|_| Error::invalid_value())?.into();
            ...
            for section in sections {
                ...
                cd.parameters.push(
                    if unicase::eq_ascii(&*key, "filename") {
                        ...
                        let extended_value = util::parse_extended_value(val)?;
            ...
            match *param {
                DispositionParam::Filename(ref charset, ref opt_lang, ref bytes) => {
                    ...
                    if use_simple_format {
                        ...
                        write!(f, "'")?;
                        util::http_percent_encode(f, bytes)?;
                    }
                },
